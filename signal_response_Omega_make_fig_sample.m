...
% load(".\Results2\RRO\signalResponse\"+num2str(divide)+"\signal_response_Omega_K_"+num2str(Kcoef(a_value))+"_RRO_"+num2str(A(a_value))+".mat");

for change=1:181
    max_C(:,change)=max_C(:,change)+hak(change);
end


map_C=mean(max_C,1);
map_C_std=std(max_C,1);
...
RRO_max_C_Omega_large_1=max_C(:,st_pos(1));
RRO_max_C_Omega_large_2=max_C(:,st_pos(2));
RRO_max_C_Omega_large_3=max_C(:,st_pos(3));
RRO_max_C_Omega_large_4=max_C(:,st_pos(4));
...
rro_line=semilogx(A_list(:),(map_C(:)),'k','Linewidth',2,'DisplayName','RRO');
xlim([1e-3 1e-1])
ylim([0 1])
ylabel({'Corr coef';'arg max_{\it\tau} Corr({\it\tau})'});
xlabel('Input frequency {\it \Omega}')

hold on;
grid on;

semilogx(A_list(:),(map_C(:))+map_C_std(:),':k','Linewidth',2);
semilogx(A_list(:),(map_C(:))-map_C_std(:),':k','Linewidth',2);
...
DG_max_C_A_large_1=max_C(:,st_pos(1));
DG_max_C_A_large_2=max_C(:,st_pos(2));
DG_max_C_A_large_3=max_C(:,st_pos(3));
DG_max_C_A_large_4=max_C(:,st_pos(4));
...
dgrro_line=semilogx(A_list(:),(map_C(:)),'r','Linewidth',2,'DisplayName',"DG-RRO"+num2str(divide));

hold on;
grid on;

semilogx(A_list(:),(map_C(:))+map_C_std(:),':r','Linewidth',2);
semilogx(A_list(:),(map_C(:))-map_C_std(:),':r','Linewidth',2);

% xlim([1e-2 0.3]);

legend('RRO','DG-RRO')
legend([rro_line dgrro_line],{'RRO','DG-RRO'})
...
xticks([A_list(st_pos(4)) A_list(st_pos(3)) A_list(st_pos(2)) A_list(st_pos(1))])

exportgraphics(f1,".\Results2\Compare\Omega\"+num2str(divide)+"\signal_response_profile_K_"+num2str(Kcoef(i_a))+ ...
    "_Omega_A_"+num2str(A(i_a))+".png",'Resolution',400,"Append",false)
close(f1)

% --------------------------------

X1=DG_max_C_A_large_1;
X2=RRO_max_C_Omega_large_1;
...
errorbar(1.4,mean(RRO_max_C_Omega_large_4),std(RRO_max_C_Omega_large_4),'*k','Linewidth',3);
...
errorbar(1.4,mean(RRO_max_C_Omega_large_3),std(RRO_max_C_Omega_large_3),'*k','Linewidth',3);
...
errorbar(1.4,mean(RRO_max_C_Omega_large_2),std(RRO_max_C_Omega_large_2),'*k','Linewidth',3);
...
errorbar(1.4,mean(RRO_max_C_Omega_large_1),std(RRO_max_C_Omega_large_1),'*k','Linewidth',3);
...
if (result_large_p_1>=0.05)
    title({"{\it t=" + num2str(result_large_1) + " (p=" + num2str(result_large_p_1) + ")}"; ...
        "{\it \Omega} = " + num2str(A_list(st_pos(1)))});
elseif (result_large_p_1>=0.001)
    title({"{\it t=" + num2str(result_large_1) + " (p=" + num2str(result_large_p_1) + ")} " + "\color{red} *"; ...
        "\color{black}{\it \Omega} = " + num2str(A_list(st_pos(1)))});
else
    title({"{\it t=" + num2str(result_large_1) + " (p < 0.001)}" + "\color{red} *"; ...
        "\color{black}{\it \Omega} = " + num2str(A_list(st_pos(1)))});
end

exportgraphics(f2,".\Results2\Compare\Omega\"+num2str(divide)+"\signal_response_K_"+num2str(Kcoef(i_a))+ ...
    "_Omega_A_"+num2str(A(i_a))+".png",'Resolution',400,"Append",false)
close(f2)
